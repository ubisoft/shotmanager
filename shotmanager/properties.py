...
from bpy.props import CollectionProperty, IntProperty, StringProperty, EnumProperty, BoolProperty, PointerProperty, FloatVectorProperty
...
class UAS_ShotManager_Shot ( PropertyGroup ):
    ...
    def getOutputFileName(self, frameIndex = -1, fullPath = False, fullPathOnly = False, rootFilePath = ""):
        return bpy.context.scene.UAS_shot_manager_props.getShotOutputFileName(  self,
    ...
    def _enabled_changed ( self, context ):
        context.scene.UAS_shot_manager_props.selected_shot_index = context.scene.UAS_shot_manager_props.getShotIndex(self)
    

    parentTakeIndex: IntProperty (
        name = "Parent Take Index",
        default = -1 )
    name: StringProperty (
        name = "Name",
        update = _shot_name_changed )
    start: IntProperty (
        name = "Start",
        update = _start_frame_changed )
    end: IntProperty (
        name = "End",
        update = _end_frame_changed )
...
class UAS_ShotManager_RenderSettings ( PropertyGroup ):
    ...
    renderAllTakes : BoolProperty (
        name = "Render All Takes",
        default = False )

    renderAllShots : BoolProperty (
        name = "Render All Shots",
        default = False )

    renderAlsoDisabled : BoolProperty (
        name = "Render Also Disabled",
        default = False )

    renderWithHandles : BoolProperty (
        name = "Render With Handles",
        default = False )
    
    writeToDisk : BoolProperty (
        name = "Write to Disk",
        default = False )
...
class UAS_ShotManager_Props ( PropertyGroup ):
   ...
   # render
    ...
    renderSettingsStill: PointerProperty (
        type = UAS_ShotManager_RenderSettings )
    
    renderSettingsAnim: PointerProperty (
        type = UAS_ShotManager_RenderSettings )
    
    renderSettingsProject: PointerProperty (
        type = UAS_ShotManager_RenderSettings )
    ...
    displayStillProps: bpy.props.BoolProperty (
        name = "Display Still Preset Properties",
        get = get_displayStillProps,
        set = set_displayStillProps,
        default = True
    )
    ...
    display_name_in_shotlist: BoolProperty (
        name = "Display Name in Shot List",
        default = True,
        options = set ( ) )

    display_camera_in_shotlist: BoolProperty (
        name = "Display Camera in Shot List",
        default = False,
        options = set ( ) )

    display_lens_in_shotlist: BoolProperty (
        name = "Display Camera Lens in Shot List",
        default = False,
        options = set ( ) )

    display_duration_in_shotlist: BoolProperty (
        name = "Display Shot Duration in Shot List",
        default = False,
        options = set ( ) )

    display_color_in_shotlist: BoolProperty (
        name = "Display Color in Shot List",
        default = False,
        options = set ( ) )

    display_selectbut_in_shotlist: BoolProperty (
        name = "Display Camera Selection Button in Shot List",
        default = True,
        options = set ( ) )
    ...
    change_time: BoolProperty (
        default = True,
        options = set ( ) )

    display_disabledshots_in_timeline: BoolProperty (
        default = False,
        options = set ( ) )
    ...
    def getTakeIndex( self, take ):
      #  print("getCurrentTakeIndex")
        ...
        #        print("  toto02: takeInd: ", takeInd)
                takeInd += 1
            if takeInd >= len (self.takes): takeInd = -1
    ...
    useProjectRenderSettings : BoolProperty (
        name = "Use Render Project Settings",
        description = "Use Render Project Settings",
        default = True,
        )
    ...
    def getEditCurrentTime( self, ignoreDisabled = False ):
        ...
        editCurrentTime = -1
        if -1 == takeInd: return editCurrentTime
    ...
    def getUniqueShotName( self, nameToMakeUnique, takeIndex = -1):
        takeInd = self.getCurrentTakeIndex() if -1 == takeIndex else (takeIndex if 0 < len(self.getTakes()) else -1)
        uniqueName = nameToMakeUnique
        if -1 == takeInd: return uniqueName
    ...
    def addShot( self, atIndex = -1, takeIndex = -1, name = "defaultShot", start = 10, end = 20, camera = None, color = ( 0.2, 0.6, 0.8, 1 ), enabled = True ):
    ...
    def getShotIndex( self, shot, takeIndex = -1 ):
        ...
        while shotInd < len(shots) and shot != shots[shotInd]:      #wkip mettre shots[shotInd].name?
            shotInd += 1
        if shotInd == len(shots): shotInd = -1
    ...
    def getCurrentShotIndex( self, ignoreDisabled = False, takeIndex = -1 ):
     ...
     #   print(" *** getCurrentShotIndex")
        ...
        takeInd = self.getCurrentTakeIndex() if -1 == takeIndex else (takeIndex if 0 < len(self.getTakes()) else -1)
        currentShotInd = -1
        if -1 == takeInd: return currentShotInd
    ...
    def getCurrentShot( self, ignoreDisabled = False, takeIndex = -1 ):
     ...
     #   print("getCurrentShot: currentShotInd: ", currentShotInd) 
        currentShot = None
        if -1 != currentShotInd: currentShot = self.takes[takeInd].shots[currentShotInd]
    ...
    def getSelectedShot( self ):
        ...
        selectedShot = None
        if -1 != selectedShotInd: selectedShot = (self.get_shots())[selectedShotInd]
    ...
    def getFirstShotIndex( self, ignoreDisabled = False, takeIndex = -1 ):
        ...
        takeInd = self.getCurrentTakeIndex() if -1 == takeIndex else (takeIndex if 0 < len(self.getTakes()) else -1)
        firstShotInd = -1
        if -1 == takeInd: return firstShotInd
    ...
    def getLastShotIndex( self, ignoreDisabled = False, takeIndex = -1 ):
        takeInd = self.getCurrentTakeIndex() if -1 == takeIndex else (takeIndex if 0 < len(self.getTakes()) else -1)
        lastShotInd = -1
        if -1 == takeInd: return lastShotInd

        shots = self.getShotsList(ignoreDisabled = ignoreDisabled, takeIndex = takeIndex)
        print(" getLastShotIndex: shots: ", shots)

        if 0 < len (shots): lastShotInd = len(shots) - 1
    ...
    def getFirstShot( self, ignoreDisabled = False, takeIndex = -1 ):
        ...
        firstShot = None if -1 == firstShotInd else self.getShot(firstShotInd, ignoreDisabled = ignoreDisabled, takeIndex = takeIndex)
    ...
    def getLastShot( self, ignoreDisabled = False, takeIndex = -1 ):
        ...
        lastShot = None if -1 == lastShotInd else self.getShot(lastShotInd, ignoreDisabled = ignoreDisabled, takeIndex = takeIndex)
    ...
    def getPreviousEnabledShotIndex( self, currentShotIndex, takeIndex = -1 ):
        takeInd = self.getCurrentTakeIndex() if -1 == takeIndex else (takeIndex if 0 < len(self.getTakes()) else -1)
        previousShotInd = -1
        if -1 == takeInd: return previousShotInd
    ...
    def getNextEnabledShotIndex( self, currentShotIndex, takeIndex = -1 ):
        takeInd = self.getCurrentTakeIndex() if -1 == takeIndex else (takeIndex if 0 < len(self.getTakes()) else -1)
        nextShotInd = -1
        if -1 == takeInd: return nextShotInd
        ...
        if len(shots) <= nextShotInd: nextShotInd = -1
    ...
    def getShotOutputFileNameFromIndex(self, shotIndex = -1, takeIndex = -1, frameIndex = -1, fullPath = False, fullPathOnly = False, rootFilePath = ""):
        shot = self.getShot(shotIndex = shotIndex, takeIndex = takeIndex)

        fileName = ""
        if None == shot: return fileName

        fileName = self.getShotOutputFileName(shot, fullPath = fullPath, fullPathOnly = fullPathOnly, rootFilePath = rootFilePath)
