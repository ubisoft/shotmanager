...
class UAS_ShotManager_ShotAdd ( Operator ):
    ...
    bl_description = "Add a new shot starting at the current frame and using the selected camera" \
...
#            camera  = scene.objects[ self.camera ] if self.camera else None,
        ...
        props.setCurrentShotByIndex(newShotInd)             # crash at very first added shot if removed...
    ...
    def invoke ( self, context, event ):
        ...
        self.name = f"{context.scene.UAS_shot_manager_props.new_shot_prefix}{len ( context.scene.UAS_shot_manager_props.getShotsList() ) + 1:02}" + "0"
        self.start = context.scene.frame_current
        self.end = context.scene.frame_current + context.scene.UAS_shot_manager_props.new_shot_duration
        self.color = (uniform(0, 1), uniform(0, 1), uniform(0, 1) )

        camName = context.scene.UAS_shot_manager_props.getActiveCameraName()
        if "" != camName: self.cameraName = camName

    #     cameras = context.scene.UAS_shot_manager_props.getSceneCameras()
...
class UAS_ShotManager_ShotDuplicate ( Operator ):
    ...
    startAtCurrentTime: BoolProperty (
        ...
        #    print(" current: " + str(currentShotInd) + ", len(shots): " + str(len(shots)) + ", selectedShotInd: " + str(selectedShotInd))
            ...
            if selectedShotInd == len(shots) - 1:
              ...
              #  props.selected_shot_index = selectedShotInd - 1
                ...
                else: props.setSelectedShotByIndex(selectedShotInd - 1)
...
class UAS_ShotManager_ShotRemoveMultiple( Operator ):
    ...
    action: bpy.props.EnumProperty ( items = ( ('ALL', "ALL", ""),
                                               ('DISABLED', "DISABLED", "") ) )
            ...
            if self.action == 'ALL':
                ...
                while i > -1:
                    if not shots[i].enabled:
                        if currentShotInd == len(shots) - 1 \
                        and currentShotInd == selectedShotInd:
                            pass
                        shots.remove(i)
                    i -= 1
...
class UAS_ShotManager_Shots_RemoveCamera ( Operator ):
    ...
    removeFromOtherTakes: BoolProperty (
        name = "Also Remove From Other Takes",
        default = False )
...
_classes = ( UAS_ShotManager_ShotAdd,
