...
def exportTakeEditToOtio(
            ...
            if shot.enabled:
                ...
                clip_start_time, clip_end_time_exclusive = (
                    ...
                    opentimelineio.opentime.from_frames(
                        shot.end - shot.start + 1 + props.getHandlesDuration(), sceneFps
                    ),
                )
                source_range = opentimelineio.opentime.range_from_start_end_time(
                    clip_start_time, clip_end_time_exclusive
                )
    ...
    if output_filepath.endswith(".xml"):
        ...
        _addEditCharacteristicsToXML(output_filepath, montageCharacteristics)
...
def exportShotManagerEditToOtio(
    ...
    for shot in shotList:
        if shot.enabled:
            ...
            shotFileFullPath = shot.getOutputMediaPath("SH_VIDEO", rootPath=renderPath, insertSeqPrefix=True)
            print(" Export otio - shotFileFullPath: ", shotFileFullPath)
            # shotFileName = Path(shotFileFullPath).name
