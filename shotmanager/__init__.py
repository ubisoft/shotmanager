...
from .scripts import RRS_StampInfo
...
class UAS_PT_ShotManager ( Panel ):
    ...
    def draw_header_preset(self, context):
        ...
        row.operator ( "uas_shot_manager.render_openexplorer", text="", icon="FILEBROWSER" ).path = bpy.path.abspath(bpy.data.filepath)
    ...
    # play and timeline
        row = layout.row ( )
        row.scale_y = 1.2
        row.prop ( context.window_manager, "UAS_shot_manager_handler_toggle",   text = "Shots Play Mode" if context.window_manager.UAS_shot_manager_handler_toggle else "Standard Play Mode",
    ...
    # play bar
        ...
        #row.ui_units_x = 40
        row.scale_x = 1.8
        row.operator ( "SCREEN_OT_animation_play", text = "", icon = "PLAY" if not context.screen.is_animation_playing else "PAUSE" )
        row.scale_x = 1
   ...
   #     row.prop(context.scene, "frame_current", text = "")            # prend la propriété direct ds la scene
    ...
    # shots
        if len ( props.takes ):
            ...
            row = box.row ( )
            templateList = row.template_list(   "UAS_UL_ShotManager_Items", "", props.getCurrentTake(),
                                                "shots", props, "selected_shot_index", rows=6 )
...
class UAS_UL_ShotManager_Items ( bpy.types.UIList ):
    ...
    ### duration    
        if item.start < currentFrame and currentFrame < item.end or (item.start == item.end and currentFrame == item.start ) :
            if props.highlight_all_shot_frames or current_shot_index == index:
                grid_flow.alert = True
...
class UAS_PT_ShotManager_ShotProperties ( Panel ):
    ...
    def draw ( self, context ):
      ...
      #  shotPropertiesModeIsCurrent = not ('SELECTED' == props.current_shot_properties_mode)
        ...
        else: shot = props.getShot(props.selected_shot_index)
...
classes = ( UAS_UL_ShotManager_Items,
            UAS_PT_ShotManager,
...
def register ( ):
    ...
    bpy.types.WindowManager.UAS_shot_manager_displayAbout = BoolProperty(
        name="About...",
        description = "Display About Informations",
        default = False )

    bpy.types.WindowManager.UAS_shot_manager_isInitialized = BoolProperty (
        name = "Shot Manager is initialized",
        description = "",                                                                                
        default = False )
