...
class UAS_ShotManager_PredecTools_CreateShotsFromSingleCamera ( Operator ):
    ...
    bl_description = "Precut Tool: Create a set of shots based on the specified camera." \
    ...
    def _set_duration(self, value):
        ...
        if value <= 1: self.duration = 1

    def _get_duration(self):
        return self.duration

    start: IntProperty (
        name = "First Shot Start",
        description = "" )
    end: IntProperty (
        name = "Last Shot Start",
        description = "" )
    ...
    cameraName: EnumProperty (
        items = list_cameras,
        name = "Camera",
        description = "Camera that will be used for every new shot" )
