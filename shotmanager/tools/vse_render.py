...
from bpy.props import IntProperty, IntVectorProperty, StringProperty, EnumProperty, BoolProperty, PointerProperty, FloatVectorProperty
...
class UAS_VSE_OpenFileBrowser( Operator ): #from bpy_extras.io_utils import ImportHelper
    ...
    bl_label    = "Open"
    bl_description  =   "Open the file browser to define the image to stamp\n" \
    ...
    filter_glob : StringProperty(
        default = '*.jpg;*.jpeg;*.png;*.tif;*.tiff;*.tga;*.mp4',
        options = {'HIDDEN'} )
...
class UAS_compositeVideoInVSE( Operator ):
    ...
    def execute(self, context):
        ...
        vseScene = context.window_manager.UAS_vse_render.compositeVideoInVSE()
        ...
        ### add BG
        scene.render.resolution_x = vse_render.inputBGResolution[0]
        scene.render.resolution_y = vse_render.inputBGResolution[1]
        scene.frame_start = 1
        scene.frame_end = 20
        scene.render.image_settings.file_format = 'FFMPEG'
        scene.render.ffmpeg.format = 'MPEG4'
        scene.render.filepath = "c:\\tmp\\" + "MyVSEOutput"
        #scene.render.ffmpeg.constant_rate_factor = video_quality

        
        def _createNewClip( mediaPath, channelInd, atFrame ):
            newClip = None
            mediaType = vse_render.getMediaType( mediaPath )
            if 'MOVIE' == mediaType:
        ...
        bgClip = _createNewClip( vse_render.inputBGMediaPath, 1, 1 )
        overClip = _createNewClip( vse_render.inputOverMediaPath, 2, 1 )
...
class UAS_Vse_Render ( PropertyGroup ):
    ...
    inputOverMediaPath: StringProperty(
        name = "Input Media Path Over",
        get = get_inputOverMediaPath,
        set = set_inputOverMediaPath,
        default = "" )
    
    inputOverResolution : IntVectorProperty(
        size = 2,
        default = (1280, 720) )
    ...
    inputBGMediaPath : StringProperty (
        name = "Input Media Path BG",
        get = get_inputBGMediaPath,
        set = set_inputBGMediaPath,
        default = "" )
    
    inputBGResolution : IntVectorProperty(
        size = 2,
        default = (1280, 960) )
