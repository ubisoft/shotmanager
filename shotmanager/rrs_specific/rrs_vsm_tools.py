...
class UAS_VideoShotManager_ImportPublishedSequence(Operator):
    ...
    bl_label = "Import Published Sequence"
    bl_description = "Import sequence generated by the Publish"
    bl_options = {"INTERNAL", "UNDO"}
    ...
    def invoke(self, context, event):
        ...
        if not Path(filePath).exists():
            ...
            sequence_AudioTrack_name = f"Sequence {sequenceName} (audio)"
            sequence_VideoTrack_name = f"Sequence {sequenceName} (video)"
